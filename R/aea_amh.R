#' Title Approximate Exchange Algorithm (AEA) with Adaptive Metropolis Hastings (AMH)
#'
#' @param formula 2-layer ERGM model formula
#' @param y_t list of observed multi-layer networks
#' @param L.x_F list of observed x_F networks
#' @param L.x_P list of observed x_P networks
#' @param L.z_F list of observed z_F networks
#' @param L.z_P list of observed z_P networks
#' @param sy observed statistics, i.e., list(sz$F_obs, sz$P_obs), include the L(~edges, ~(active))
#' @param iters overall number of iterations for each MCMC chain
#' @param burn.in number of burnin iterations for each MCMC chain
#' @param ergm_sim_iters number of iterations for ERGM simulation
#' @param mu.prior_F prior mean for zeta_F
#' @param Sigma.prior_F prior covariance for zeta_F
#' @param mu.prior_P prior mean for zeta_P
#' @param Sigma.prior_P prior covariance for zeta_P
#' @param n_chains number of MCMC chains, default = 1
#' @param init_s_F initial scaling factor for proposal covariance for the F process
#' @param init_s_P initial scaling factor for proposal covariance for the P process
#' @param adapt_start when to start adaptation
#' @param adapt_interval how often to update covariance
#'
#' @returns posterior samples of theta_F and theta_P
#' @import ergm ergm.multi mvtnorm
#' @importFrom stats cov runif simulate
#' @export
aea_amh <- function(formula,
                    y_t = y_t,
                    L.x_F = L.x_F,
                    L.x_P = L.x_P,
                    L.z_F = L.z_F,
                    L.z_P = L.z_P,
                    sy = sy,
                    iters = 50000,
                    burn.in = 100,
                    ergm_sim_iters = 1000,
                    # Prior specification
                    mu.prior_F = NULL,
                    Sigma.prior_F = NULL,
                    mu.prior_P = NULL,
                    Sigma.prior_P = NULL,
                    # AMH settings
                    n_chains = 1,
                    init_s_F = 0.025,
                    init_s_P = 0.025,
                    adapt_start = 100,
                    adapt_interval = 50){

  model.dim <- dim(sy$F_obs)[1] - 1 # remove first term L(~edges, ~(active))
  times <- length(L.x_F) + 1        # including t_0
  #
  if(is.null(mu.prior_F))    mu.prior_F <- rep(0, model.dim)
  if(is.null(Sigma.prior_F)) Sigma.prior_F <- diag(10, model.dim)
  if(is.null(mu.prior_P))    mu.prior_P <- rep(0, model.dim)
  if(is.null(Sigma.prior_P)) Sigma.prior_P <- diag(10, model.dim)
  #
  theta_F <- array(NA, c(iters - burn.in, model.dim, n_chains))
  theta_P <- array(NA, c(iters - burn.in, model.dim, n_chains))

  # Storage for tracking adaptive cov
  chains_F <- list()
  chains_P <- list()

  # Initialize covariance matrices for proposals
  for(h in 1:n_chains) {
    chains_F[[h]] <- list(
      samples = matrix(NA, iters, model.dim),   # Store all samples for adaptation
      cov = diag(init_s_F^2, model.dim),        # Initial proposal covariance
      s = init_s_F                              # Scaling factor
    )
    chains_P[[h]] <- list(
      samples = matrix(NA, iters, model.dim),   # Store all samples for adaptation
      cov = diag(init_s_P^2, model.dim),        # Initial proposal covariance
      s = init_s_P                              # Scaling factor
    )
  }

  # Initialize coefficients
  coef_F <- matrix(mu.prior_F +
                     runif(model.dim * n_chains,
                           min = -0.1, max = 0.1),
                   model.dim, n_chains)

  coef_P <- matrix(mu.prior_P +
                     runif(model.dim * n_chains,
                           min = -0.1, max = 0.1),
                   model.dim, n_chains)

  # Adaptation constant
  sd_scale <- 2.4^2/model.dim

  message(" > MCMC start")

  for (i in 1:iters) {
    for (h in 1:n_chains) {

      # Store current state
      chains_F[[h]]$samples[i,] <- coef_F[, h]
      chains_P[[h]]$samples[i,] <- coef_P[, h]

      # Adaptive proposal - after adaptation start
      if (i > adapt_start) {
        # Update covariance periodically
        if (i %% adapt_interval == 0) {
          # Calculate empirical covariance of chain history with small regularization
          # Formation
          emp_cov_F <- cov(chains_F[[h]]$samples[1:i,]) + diag(1e-6, model.dim)
          chains_F[[h]]$cov <- sd_scale * emp_cov_F
          # Persistence
          emp_cov_P <- cov(chains_P[[h]]$samples[1:i,]) + diag(1e-6, model.dim)
          chains_P[[h]]$cov <- sd_scale * emp_cov_P
        }
      }

      # Propose new values using current covariance
      coef._F <- coef_F[, h] + rmvnorm(1, sigma = chains_F[[h]]$cov)[1, ]
      coef._P <- coef_P[, h] + rmvnorm(1, sigma = chains_P[[h]]$cov)[1, ]

      # Simulate statistics for proposed parameters
      sy. <- simulate_stats(formula,
                            y_t = y_t,
                            L.x_F = L.x_F,
                            L.x_P = L.x_P,
                            L.z_F = L.z_F,
                            L.z_P = L.z_P,
                            times = times,
                            model.dim = model.dim + 1, # include (Inf,...
                            coef_F = coef._F,
                            coef_P = coef._P,
                            ergm_sim_iters = ergm_sim_iters)

      # Calculate differences between simulated and observed statistics
      delta_F <- rowSums(sy.$sz._F - sy$F_obs)[-1]
      delta_P <- rowSums(sy.$sz._P - sy$P_obs)[-1]

      # Calculate log prior densities
      prior_F <- dmvnorm(rbind(coef._F, coef_F[, h]),
                         mean = mu.prior_F,
                         sigma = Sigma.prior_F,
                         log = TRUE)

      prior_P <- dmvnorm(rbind(coef._P, coef_P[, h]),
                         mean = mu.prior_P,
                         sigma = Sigma.prior_P,
                         log = TRUE)

      # Calculate log acceptance probabilities
      beta_F <- (coef_F[, h] - coef._F) %*% delta_F + prior_F[1] - prior_F[2]
      beta_P <- (coef_P[, h] - coef._P) %*% delta_P + prior_P[1] - prior_P[2]

      # Accept/reject for Formation model
      if (c(beta_F) >= log(runif(1))) {
        coef_F[, h] <- coef._F
      }

      # Accept/reject for Persistence model
      if (c(beta_P) >= log(runif(1))) {
        coef_P[, h] <- coef._P
      }
    }

    # Store post burn-in samples
    if (i > burn.in){
      theta_F[i - burn.in, , ] <- coef_F
      theta_P[i - burn.in, , ] <- coef_P
    }

    # Periodic progress updates
    if (i %in% seq(100, iters, 100)){
      message(" >", i)
    }
  }

  # Return results
  list(theta_F = theta_F,
       theta_P = theta_P,
       chains_F = chains_F,
       chains_P = chains_P)
}
